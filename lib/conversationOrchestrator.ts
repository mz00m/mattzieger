...
function buildConversationHistory(
  ...
  if (action === 'user-spoke' && userInput) {
    ...
    messages.push({
      role: 'user',
      content: [
        'Keep the conversation going. 4-6 exchanges.',
        'Let it drift naturally — someone changes the angle, someone teases,',
        'someone gets curious. Like real dinner conversation.',
        'Most lines are 1-3 sentences. But if someone has a great story, joke, or',
        'anecdote, let them tell it fully (4-8 sentences). Comedians tell actual jokes',
        'with setups and punchlines. Storytellers tell real anecdotes.',
        'Other guests react during stories: "[laughs]", "And then what?", "No way."',
        'Include messy human stuff: laughing, trailing off, interrupting.',
        '',
        'JSON array: { "type": "speech", "speaker": "figure-id", "text": "..." }',
        'Return ONLY valid JSON.',
      ].join('\n'),
